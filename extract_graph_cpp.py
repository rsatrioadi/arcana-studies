#!/usr/bin/env python3
"""
extract_graph_cpp.py — Run cpppers over every repo in a source directory.

For each subdirectory <repo> under <sources>, executes:

    cpppers --name <repo> --format cyjson --no-compile-commands <sources>/<repo>

stdout is captured and written to <output_dir>/<repo>.json.

Resume: skips repos whose output file already exists.
Failures are logged to <output_dir>/_failed.txt.

Usage:
  python extract_graph_cpp.py --sources repos/src/cpp --output-dir results
  python extract_graph_cpp.py --sources repos/src/cpp --output-dir results \\
