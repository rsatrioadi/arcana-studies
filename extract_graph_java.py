#!/usr/bin/env python3
"""
extract_graph_java.py — Run a Java analysis jar over repos listed in a roots CSV.
...
Usage:
  python extract_graph_java.py --csv java_roots.csv --jar saboroot.jar
  python extract_graph_java.py --csv java_roots.csv --jar saboroot.jar \\
...
def load_csv(csv_path: Path) -> list[dict]:
    ...
    for col in ("repo", "source_paths"):
        if col not in rows[0]:
            sys.exit(
                f"Error: CSV missing column '{col}'. "
...
def load_failed(output_dir: Path) -> set[str]:
    ...
    if not failed_file.exists():
        return set()
    return {
        line.strip() for line in failed_file.read_text().splitlines() if line.strip()
    }
...
def remove_from_failed(output_dir: Path, repo: str):
    ...
    lines = [
        l
        for l in failed_file.read_text().splitlines()
...
def run_one(
    repo: str, source_paths: str, args: argparse.Namespace, force: bool = False
) -> tuple[str, bool, str]:
    """
